...
open class UDPBroadcastConnection {
     ...
     */
    public init(remotePort: UInt16, localPort: UInt16, handler: ((_ ipAddress: String, _ port: Int, _ response: [UInt8]) -> Void)?) {
        self.remoteAddress = sockaddr_in(
            sin_len:    __uint8_t(MemoryLayout<sockaddr_in>.size),
            sin_family: sa_family_t(AF_INET),
            sin_port:   UDPBroadcastConnection.htonsPort(port: remotePort),
            sin_addr:   INADDR_BROADCAST,
            sin_zero:   ( 0, 0, 0, 0, 0, 0, 0, 0 )
        )

        self.localAddress = sockaddr_in(
            sin_len:    __uint8_t(MemoryLayout<sockaddr_in>.size),
            sin_family: sa_family_t(AF_INET),
            sin_port:   UDPBroadcastConnection.htonsPort(port: localPort),
            sin_addr:   INADDR_ANY,
            sin_zero:   ( 0, 0, 0, 0, 0, 0, 0, 0 )
        )
    ...
    func createSocket() -> Bool {
        ...
        var ret = setsockopt(newSocket, SOL_SOCKET, SO_BROADCAST, &broadcastEnable, socklen_t(MemoryLayout<UInt32>.size));
        if ret == -1 {
        ...
        let socketLength = socklen_t(self.localAddress.sin_len)
        
        ret = Int32(withUnsafeMutablePointer(to: &self.localAddress) { pointer -> Int32 in
            let memory = UnsafeRawPointer(pointer).bindMemory(to: sockaddr.self, capacity: 1)
            
            return bind(newSocket, memory, socketLength)
        })
    ...
    open func sendBroadcast(_ data: Data) {
        ...
        let UDPSocket = Int32(source.handle)
        let socketLength = socklen_t(remoteAddress.sin_len)
        data.withUnsafeBytes { (broadcastMessage: UnsafePointer<Int8>) in
            let broadcastMessageLength = data.count
            let sent = withUnsafeMutablePointer(to: &remoteAddress) { pointer -> Int in
                let memory = UnsafeRawPointer(pointer).bindMemory(to: sockaddr.self, capacity: 1)
